...
from pprzlink.xbee_transport import XbeeTransport
...
class SerialMessagesInterface(threading.Thread):
    ...
    def run(self):
        ...
        try:
            while self.running:
                # Parse incoming data
                cs = self.ser.read_all()
                if cs is not None and len(cs) > 0:
                    msgs = self.trans.parse_bytes(cs)
                    for (sender_id, msg, receiver_id, component_id) in msgs:
                ...
                #         try:
                #             (sender_id, msg, receiver_id, component_id) = self.trans.unpack()
...
class XbeeMessagesInterface(threading.Thread):
    ...
    def __init__(self, callback, verbose=False, device='/dev/ttyUSB0', baudrate=57600,
        ...
        self.msg_class = msg_class
        self.trans = XbeeTransport(msg_class)
    ...
    def send(self, msg, sender_id:typing.Optional[int]=None, receiver_id:int=0, component_id:int=0):
        """ Send a message over a serial link"""
        if sender_id is None:
            sender_id = self.id
        if isinstance(msg, PprzMessage):
    ...
    def get_response_timeout(self, frame_id, timeout=DEFAULT_TIMEOUT) -> Any:
        ...
        while frame_id not in self.trans.responses:
            if time.time() - start_time > timeout:
                raise TimeoutError()
            time.sleep(0.01)
        return self.trans.responses.pop(frame_id)
        
    def at_cmd(self, at:bytes, value:typing.Optional[bytes]=None, resp:bool=True):
        cmd,id = self.trans.at_cmd(at,value,resp)
        self.ser.write(cmd)
        self.ser.flush()
        return self.get_response_timeout(id)
...
def test():
    ...
    parser.add_argument("-t", "--transport", help="Name of the transport to use", dest='transport', default="xbee", choices=['pprz','xbee'])
    ...
    if args.transport == "pprz":
        ...
        interface = XbeeMessagesInterface(lambda s, m: print("new message from %i: %s" % (s, m)), 
                                          interface_id=args.id, device=args.dev, baudrate=args.baud, verbose=True,
